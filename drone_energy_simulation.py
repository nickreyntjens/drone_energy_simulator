...
def slow_down(drone, target_speed, dt, simulation_history):
    """
    Slow down the drone until its speed is at or below target_speed.
    The drone is decelerated using its maximum acceleration in the direction opposite to its velocity.
    
    Parameters:
      - drone: instance of Drone.
      - target_speed: desired max speed (in m/s) for locking.
      - dt: simulation time step.
      - simulation_history: list to record simulation states.
    """
    current_speed = np.linalg.norm(drone.velocity)
    # Only slow down if the drone is moving faster than the target speed.
    while current_speed > target_speed and drone.total_time < 1e6:  # prevent an infinite loop in extreme cases
        # Apply deceleration opposite to the current velocity.
        acc = -drone.velocity / current_speed * drone.max_acc
        drone.update(acc, dt)
        current_speed = np.linalg.norm(drone.velocity)
        simulation_history.append({
            'time': drone.total_time,
    ...
    while (remaining_insects or state in ("goto_recharge", "recharge")) and drone.total_time < active_period:
        ...
        if state == "target_insect":
            ...
            if reached:
                ...
                if engaged:
                    # Slow down before locking if current speed exceeds max_lock_speed.
                    max_lock_speed_kmh = drone_params.get("max_speed_when_shooting_kmh", 3.0)
                    max_lock_speed_ms = max_lock_speed_kmh / 3.6  # convert km/h to m/s
                    current_speed_ms = np.linalg.norm(drone.velocity)
                    if current_speed_ms > max_lock_speed_ms:
                        print(f"Slowing down before lock: current speed {current_speed_ms*3.6:.2f} km/h, target {max_lock_speed_kmh:.2f} km/h")
                        slow_down(drone, max_lock_speed_ms, dt, simulation_history)
...
drone_params = {
    ...
    'max_speed_when_shooting_kmh': 3.0,  # Maximum speed (km/h) allowed during locking
...
def drone_config_window(event):
    ...
    fig_drone = plt.figure("Drone Configuration Settings", figsize=(7, 11))
    
    # Define slider positions (from top to bottom)
    ax1 = fig_drone.add_axes([0.15, 0.90, 0.7, 0.05])  # Max Acceleration
    ax2 = fig_drone.add_axes([0.15, 0.82, 0.7, 0.05])  # Max Speed
    ax3 = fig_drone.add_axes([0.15, 0.74, 0.7, 0.05])  # Energy Consumption
    ax4 = fig_drone.add_axes([0.15, 0.66, 0.7, 0.05])  # Battery (mAh)
    ax5 = fig_drone.add_axes([0.15, 0.58, 0.7, 0.05])  # Laser Energy
    ax6 = fig_drone.add_axes([0.15, 0.50, 0.7, 0.05])  # Low Battery Fraction
    ax7 = fig_drone.add_axes([0.15, 0.42, 0.7, 0.05])  # Lock Time (s)
    ax8 = fig_drone.add_axes([0.15, 0.34, 0.7, 0.05])  # Engagement Range (m)
    ax9 = fig_drone.add_axes([0.15, 0.26, 0.7, 0.05])  # Max Lock Speed (km/h)
    ...
    ax_apply = fig_drone.add_axes([0.70, 0.02, 0.25, 0.05])
    btn_apply = Button(ax_apply, "Apply")
    ...
    def apply_drone_settings(event):
        ...
        drone_params['max_speed_when_shooting_kmh'] = slider_max_lock_speed.val
        drone_params['num_lasers'] = int(slider_num_lasers.val)
        drone_params['num_cells'] = int(slider_num_cells.val)
...
def show_help_window(event):
    ...
    help_text = (
        ...
        "  max_speed_when_shooting_kmh: Speed threshold for locking (km/h).\n"
